...
/* toolbar */
    this->
            tb = new Toolbar(this, 0, 50, 1920, 50);
...
/* 关闭按钮 */
    ...
    connect(closeBtn, &QToolButton::clicked,
            this, &BrowserWindow::on_closeBtn_clicked);
...
/* 放大缩小按钮 */
    ...
    connect(minBtn, &QToolButton::clicked,
            this, &BrowserWindow::on_minBtn_clicked);
...
/* 最小化按钮 */
    ...
    connect(hidBtn, &QToolButton::clicked,
            this, &BrowserWindow::on_hidBtn_clicked);


// 初始化历史界面并隐藏
    qDebug()

            << "[test] browserwindown slot1 init";
    this->
            historyTest = new HistoryWidget(this, 0, 100, 1920, 980, this);
    qDebug()

            << "[test] browserwindown slot2 init";
    this->historyTest->

            hide();

    if (tb) {
        connect(tb, &Toolbar::on_historyBtn_passSignal,
                this, &BrowserWindow::accept_history_signal);
    }

// 初始化书签界面并隐藏
    this->
            bookmarkTest = new BookmarkWidget(this, 0, 100, 300, 980, this);
    this->bookmarkTest->

            hide();

    if (tb) {
        connect(tb, &Toolbar::on_bookmarkerBtn_passSignal,
                this, &BrowserWindow::accept_bookmarker_signal);
    }
